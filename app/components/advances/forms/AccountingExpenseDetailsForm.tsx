...
export default function AccountingExpenseDetailsForm({
  ...
  const handleChange = (index: number, field: string, value: any) => {
    if (
      ...
      return;
    const draftAccountingLines = [
      ...selectedAdvanceLineForViewAccountingDetails,
    ];
    const updatedLine = draftAccountingLines[index];
    if (safeTypechecker(updatedLine) === "Object") {
      updatedLine[field] = isNaN(Number(value)) ? value : Number(value);
  ...
  return (
    <table className="table table-bordered my-3 align-middle">
      ...
      <tbody>
        {selectedAdvanceLineForViewAccountingDetails.map(
          (exp: Record<string, any>, idx: number) => (
            <tr
              key={`${selectedAdvanceLineForView?.expenseCode}-${
            ...
            >
              ...
              <td className="d-flex justify-content-center align-items-center">
                ...
                <button
                  ...
                  onClick={async () =>
                    await deleteDetailedExpesneLine(idx, exp)
                  }
                >
