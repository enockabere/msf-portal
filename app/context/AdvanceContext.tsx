...
const initialState = {
  ...
  accountingLinesByLineNo: {} as Record<number, Record<string, any>[]>,
  actions: {
    ...
    /* eslint-disable @typescript-eslint/no-unused-vars */
    handleFetchingSetup: (): Promise<void> => {
      return Promise.resolve();
    },
    /* eslint-disable @typescript-eslint/no-unused-vars */
    fetchLineSetup: (): Promise<void> => {
      return Promise.resolve();
    },
  },
};
...
function AdvanceReducer(
  state: AdvanceState,
  action: ReducerFunctionActionType
) {
  switch (action.type) {
...
export const AdvanceContextProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
